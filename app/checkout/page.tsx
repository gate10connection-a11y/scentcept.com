...
export default function CheckoutPage() {
    ...
    return (
      <div>
        ...
        <div className="text-center py-24">
          ...
          <Link href="/collections" className="text-[10px] tracking-[0.2em] uppercase border-b border-black pb-0.5">Continue Shopping</Link>
        </div>
      </div>
    );
  }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (error) { alert("Checkout error: " + error); setLoading(false); return; }
      if (url) { window.location.href = url; }
      else { window.location.href = "/order-confirmation?demo=true"; }
    } catch {
      ...
      <div className="grid md:grid-cols-2">
        <div className="md:border-r md:border-black/15">
          ...
          <div className="p-6 md:p-10">
            ...
            >{loading ? "Processing..." : "Complete Order"}</button>
          </div>
        </div>
            ...
            <div className="space-y-5">
              {state.items.map((item) => (
                <div key={`${item.product.id}-${item.size}`} className="flex gap-4 items-start">
                  <div className="relative w-14 md:w-16 aspect-[3/4] bg-[#f5f5f5] flex-shrink-0">
                    <Image src={item.product.images[0]} alt={item.product.name} fill className="object-cover" sizes="64px" />
                    <span className="absolute -top-2 -right-2 w-5 h-5 bg-black text-white text-[9px] flex items-center justify-center">{item.quantity}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] truncate">{item.product.name}</p>
                    <p className="text-[10px] text-gray-400 mt-1">Size: {item.size}</p>
                  </div>
                  <p className="text-[12px] whitespace-nowrap">{formatPrice(item.product.price * item.quantity)}</p>
                </div>
              ))}
            </div>
          </div>
