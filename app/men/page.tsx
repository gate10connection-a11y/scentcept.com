...
function MenContent() {
  ...
  const allMen = getProductsByGender("men");
  const categories = ["All", ...Array.from(new Set(allMen.map((p) => p.category)))];
  const filtered = selectedCategory === "All" ? allMen : allMen.filter((p) => p.category === selectedCategory);

  return (
    <div>
      <div className="flex items-center justify-between px-6 md:px-10 h-[44px] ab-b">
        <span style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase" }}>Shop Men</span>
...
export default function MenPage() {
  return <Suspense><MenContent /></Suspense>;
}
